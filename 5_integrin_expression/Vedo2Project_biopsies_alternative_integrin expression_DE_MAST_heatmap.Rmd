---
title: "vedo2_biopsies_check_alternative_integrins_DE_MAST_heatmap"
...
# alpha M, alpha X, beta 1, beta 2) in vedo2 scRNA-seq biopsies using MAST method
# Date: "11-10-2022"
...
alternative_integrin_list <- function(data) {
  ...
  data_ITGA1 <- data %>% 
    filter(integrin == "ITGA1") %>% 
    select(avg_log2FC, celltype, integrin)
  data_ITGA3 <- data %>% 
    filter(integrin == "ITGA3") %>% 
    select(avg_log2FC, celltype, integrin)
  data_ITGAE <- data %>% 
  ...
  return(list(data_ITGA1, data_ITGA3, data_ITGAE, data_ITGAL, data_ITGAM, data_ITGAX, data_ITGB1, data_ITGB2))
}
...
rename_columns <- function(data, suffix) {
  ...
  new_names_A1_avg_log2FC <- paste0("avg_log2FC_", suffix, "_A1")
  new_names_A1_integrin <- paste0("integrin_", suffix, "_A1")
  new_names_A3_avg_log2FC <- paste0("avg_log2FC_", suffix, "_A3")
  new_names_A3_integrin <- paste0("integrin_", suffix, "_A3")
  ...
  colnames(alternative_integrin_data[[1]]) <- c(new_names_A1_avg_log2FC, "celltype", new_names_A1_integrin)
  colnames(alternative_integrin_data[[2]]) <- c(new_names_A3_avg_log2FC, "celltype", new_names_A3_integrin)
  colnames(alternative_integrin_data[[3]]) <- c(new_names_AE_avg_log2FC, "celltype", new_names_AE_integrin)
  colnames(alternative_integrin_data[[4]]) <- c(new_names_AL_avg_log2FC, "celltype", new_names_AL_integrin)
  colnames(alternative_integrin_data[[5]]) <- c(new_names_AM_avg_log2FC, "celltype", new_names_AM_integrin)
  colnames(alternative_integrin_data[[6]]) <- c(new_names_AX_avg_log2FC, "celltype", new_names_AX_integrin)
  colnames(alternative_integrin_data[[7]]) <- c(new_names_B1_avg_log2FC, "celltype", new_names_B1_integrin)
  colnames(alternative_integrin_data[[8]]) <- c(new_names_B2_avg_log2FC, "celltype", new_names_B2_integrin)
...
idx <- seq(from = 3, to = 59, by = 2)

# Create a vector of values to assign to the specified indices
vals <- c("ITGA1", "ITGA1", "ITGAE", "ITGAL", "ITGAX", "ITGB1", "ITGB2", 
          "ITGA1", "ITGAE", "ITGAL", "ITGAM", "ITGAX", "ITGB1", "ITGB2",
          "before_ITGA1", "before_ITGA3", "before_ITGAE", "before_ITGAL", "before_ITGAM", "before_ITGAX", "before_ITGB1", "before_ITGB2", 
          "after_ITGA1", "after_ITGAE", "after_ITGAL", "after_ITGAM", "after_ITGAX", "after_ITGB1", "after_ITGB2")
...
all_comparisons_alternative_integrin$celltype <- fct_relevel(
  ...
  "CD8+ LP", "CD8+ IL17+", "CD8+ IELs", "Tregs", "CD4+ Activated Fos-lo", 
  "CD4+ Activated Fos-hi", "DC2", "NKs", "Inflammatory Monocytes"
)

# Create a new dataframe for afterR_vs_beforeR
afterR_vs_beforeR_alternative_integrin <- rbind(
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterR_vs_beforeR_A1", "integrin_afterR_vs_beforeR_A1", c(-0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterR_vs_beforeR_A3", "integrin_afterR_vs_beforeR_A3", c(-0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterR_vs_beforeR_AE", "integrin_afterR_vs_beforeR_AE", c(-0.5, 0, 0.5, 1)),
  ...
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterR_vs_beforeR_AX", "integrin_afterR_vs_beforeR_AX", c(-0.5, 0, 0.5, 1)),
...
afterNR_vs_beforeNR_alternative_integrin <- rbind(
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_beforeNR_A1", "integrin_afterNR_vs_beforeNR_A1", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_beforeNR_AE", "integrin_afterNR_vs_beforeNR_AE", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_beforeNR_AL", "integrin_afterNR_vs_beforeNR_AL", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_beforeNR_AM", "integrin_afterNR_vs_beforeNR_AM", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_beforeNR_AX", "integrin_afterNR_vs_beforeNR_AX", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_beforeNR_B1", "integrin_afterNR_vs_beforeNR_B1", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_beforeNR_B2", "integrin_afterNR_vs_beforeNR_B2", c(-1, -0.5, 0, 0.5, 1))
)

# Create a new dataframe for beforeNR_vs_beforeR
beforeNR_vs_beforeR_alternative_integrin <- rbind(
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_beforeNR_vs_beforeR_A1", "integrin_beforeNR_vs_beforeR_A1", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_beforeNR_vs_beforeR_A3", "integrin_beforeNR_vs_beforeR_A3", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_beforeNR_vs_beforeR_AE", "integrin_beforeNR_vs_beforeR_AE", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_beforeNR_vs_beforeR_AL", "integrin_beforeNR_vs_beforeR_AL", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_beforeNR_vs_beforeR_AM", "integrin_beforeNR_vs_beforeR_AM", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_beforeNR_vs_beforeR_AX", "integrin_beforeNR_vs_beforeR_AX", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_beforeNR_vs_beforeR_B1", "integrin_beforeNR_vs_beforeR_B1", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_beforeNR_vs_beforeR_B2", "integrin_beforeNR_vs_beforeR_B2", c(-1, -0.5, 0, 0.5, 1))
)

# Create a new dataframe for afterNR_vs_afterR
afterNR_vs_afterR_alternative_integrin <- rbind(
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_afterR_A1", "integrin_afterNR_vs_afterR_A1", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_afterR_AE", "integrin_afterNR_vs_afterR_AE", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_afterR_AL", "integrin_afterNR_vs_afterR_AL", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_afterR_AM", "integrin_afterNR_vs_afterR_AM", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_afterR_AX", "integrin_afterNR_vs_afterR_AX", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_afterR_B1", "integrin_afterNR_vs_afterR_B1", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_alternative_integrin, "avg_log2FC_afterNR_vs_afterR_B2", "integrin_afterNR_vs_afterR_B2", c(-1, -0.5, 0, 0.5, 1))
)
...
NR_vs_R_alternative_integrin$integrin <- factor(
  NR_vs_R_alternative_integrin$integrin,
  levels = c(
    "before_ITGA1","after_ITGA1",
    "before_ITGA3",
    "before_ITGAE", "after_ITGAE",
    "before_ITGAL", "after_ITGAL",
    "before_ITGAM", "after_ITGAM",
    "before_ITGAX", "after_ITGAX", 
    "before_ITGB1", "after_ITGB1", 
    "before_ITGB2", "after_ITGB2"))
# plot heatmap
colors1 <- c(
  "#f0c9c9", # (-0.5,0]
...
ggsave("vedo2_scRNA_biopsies_alternative_integrin_heatmapi.pdf",  plot = p_out, width=10, height=10, path = output_loc)
