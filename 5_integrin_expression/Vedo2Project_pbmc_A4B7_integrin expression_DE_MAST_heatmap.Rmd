---
title: "vedo2_pbmc_check_alpha_4_and_beta_7_integrins_DE_MAST_heatmap"
...
# in vedo2 scRNA-seq PBMCs using MAST method
# Date: "11-10-2022"
...
all_comparisons_A4B7$celltype <- fct_relevel(
  ...
  "Plasmablast", "B naive", "B memory", "CD8 TEM", 
  "CD8 Naive", "Treg", "CD4 TEM", "CD4 TCM", "CD4 Naive", 
  "CD4 CTL", "pDC", "cDC2", "ASDC", "NK", "CD14 Mono", "HSPC"
)

# Create a new dataframe for afterR_vs_beforeR
afterR_vs_beforeR_A4B7 <- rbind(
  create_grouped_data(all_comparisons_A4B7, "avg_log2FC_afterR_vs_beforeR_A4", "integrin_afterR_vs_beforeR_A4", c(0, 0.5)),
  create_grouped_data(all_comparisons_A4B7, "avg_log2FC_afterR_vs_beforeR_B7", "integrin_afterR_vs_beforeR_B7", c(0, 0.5))
)

# Create a new dataframe for afterNR_vs_beforeNR
afterNR_vs_beforeNR_A4B7 <- rbind(
  create_grouped_data(all_comparisons_A4B7, "avg_log2FC_afterNR_vs_beforeNR_A4", "integrin_afterNR_vs_beforeNR_A4", c(-1, -0.5, 0, 0.5)),
  create_grouped_data(all_comparisons_A4B7, "avg_log2FC_afterNR_vs_beforeNR_B7", "integrin_afterNR_vs_beforeNR_B7", c(-1, -0.5, 0, 0.5))
)

# Create a new dataframe for beforeNR_vs_beforeR
beforeNR_vs_beforeR_A4B7 <- rbind(
  create_grouped_data(all_comparisons_A4B7, "avg_log2FC_beforeNR_vs_beforeR_A4", "integrin_beforeNR_vs_beforeR_A4", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_A4B7, "avg_log2FC_beforeNR_vs_beforeR_B7", "integrin_beforeNR_vs_beforeR_B7", c(-1, -0.5, 0, 0.5, 1))
)

# Create a new dataframe for afterNR_vs_afterR
afterNR_vs_afterR_A4B7 <- rbind(
  create_grouped_data(all_comparisons_A4B7, "avg_log2FC_afterNR_vs_afterR_A4", "integrin_afterNR_vs_afterR_A4", c(-1, -0.5, 0, 0.5, 1)),
  create_grouped_data(all_comparisons_A4B7, "avg_log2FC_afterNR_vs_afterR_B7", "integrin_afterNR_vs_afterR_B7", c(-1, -0.5, 0, 0.5, 1))
)
...
ggsave("vedo2_scRNA_pbmc_integrin_heatmap.pdf",  plot = p_out, width=10, height=10, path = output_loc)
