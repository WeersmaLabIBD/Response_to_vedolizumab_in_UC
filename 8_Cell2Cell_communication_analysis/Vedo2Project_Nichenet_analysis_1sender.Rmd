---
title: "Nichenet analysis 1sender"
author: "Paola Pibiri"
...
# Name: Vedo2Project_Nichenet_analysis_1sender.Rmd
# Function: check the cell-cell communications between 1 sender cell and multiple receiver cells 
############################################################################################################################
```
...
## Perform the Nichenet analysis 1sender
```{r foor loop executing the analysis 1sender, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)

analyses <- list(
  'Inflammatory Monocytes' = list(
    'folder' = '/groups/umcg-weersma/tmp01/projects/vedopredict2/biopsies/ongoing/Nichenet07/Inf_mono_nichenet/'
  ),
  'Tregs' = list(
    'folder' = '/groups/umcg-weersma/tmp01/projects/vedopredict2/biopsies/ongoing/Nichenet07/Tregs_nichenet/'
  ),
  'NKs' = list(
    'folder' = '/groups/umcg-weersma/tmp01/projects/vedopredict2/biopsies/ongoing/Nichenet07/NK_nichenet/'
  ),
  'Post-capillary Venules' = list(
    'folder' = '/groups/umcg-weersma/tmp01/projects/vedopredict2/biopsies/ongoing/Nichenet07/Post_capillary_nichenet/'
  ),
  'DCs' = list(
    'folder' = '/groups/umcg-weersma/tmp01/projects/vedopredict2/biopsies/ongoing/Nichenet07/DC_nichenet/'
  ),
  'IgG' = list(
    'folder' = '/groups/umcg-weersma/tmp01/projects/vedopredict2/biopsies/ongoing/Nichenet07/IgG_nichenet/'
  ),
  'M cells' = list(
    'folder' = '/groups/umcg-weersma/tmp01/projects/vedopredict2/biopsies/ongoing/Nichenet07/M_cells_nichenet/'
  ),
  'CD8+ IL17+' = list(
    'folder' = '/groups/umcg-weersma/tmp01/projects/vedopredict2/biopsies/ongoing/Nichenet07/CD8_IL17_nichenet/'
  ),
  'Inflammatory Fibroblasts' = list(
    'folder' = '/groups/umcg-weersma/tmp01/projects/vedopredict2/biopsies/ongoing/Nichenet07/Inf_fibro_nichenet/'
  )
...
for (celltype in names(analyses)) {
  ...
  tryCatch(
    {
  ## receiver
      receiver = names(analyses)
      Idents(object = object.sub) <- 'celltype_final.v1'
      list_expressed_genes_receiver = receiver %>% unique() %>% lapply(get_expressed_genes, object.sub, 0.10)
      expressed_genes_receiver = list_expressed_genes_receiver %>% unlist() %>% unique()
      background_expressed_genes = expressed_genes_receiver %>% .[. %in% rownames(ligand_target_matrix)]
      
      ## sender
      sender_celltypes = celltype
      Idents(object = object.sub) <- 'celltype_final.v1'
      ...
      DE_table_receiver <- list()
      
      for (cell in receiver) {
        seurat_obj_receiver <- object.sub[ ,object.sub@meta.data$celltype_final.v1 == cell]
        seurat_obj_receiver = SetIdent(seurat_obj_receiver, value = seurat_obj_receiver[["PGA._resp"]])
        DE_table <- FindMarkers(object = seurat_obj_receiver, ident.1 = condition_oi, ident.2 = condition_reference, min.pct = 0.10) %>% rownames_to_column("gene")
        DE_table$celltype <- cell
        DE_table_receiver[[cell]] <- DE_table
        
        print(paste0('doing DE for ', cell))
      }
      
      DE_table_receiver_all <- do.call("rbind", DE_table_receiver)
      
      
      geneset_oi = DE_table_receiver_all %>% filter(p_val_adj <= 0.05 & abs(avg_log2FC) >= 0.25) 
      
      write.table(geneset_oi, paste0(analyses[[celltype]][['folder']], '/DE_genes_receiver_cells.tsv'), sep = '\t', col.names = T, row.names = F)
      
      print(paste0('writing table for ', celltype))
      
      geneset_oi = geneset_oi[!duplicated(geneset_oi[,c('gene')]),] %>% pull(gene)
