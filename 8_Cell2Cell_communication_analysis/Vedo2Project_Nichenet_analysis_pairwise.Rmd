---
title: "Nichenet Pairwise analysis"
author: "Paola Pibiri"
...
# Name: Vedo2Project_Nichenet_analysis_pairwise.Rmd
...
## Perform the Nichenet analysis Pairwise
```{r foor loop executing the analysis 1receiver 1sender, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)

#define the list of cells to perform the analysis
ist3_vedo2 <- c('M cells', 'ILCs', 'Inflammatory Monocytes', 'NKs', 'Tregs', 'Inflammatory Fibroblasts', 'Post-capillary Venules', 'DCs', 'IgG',  'CD8+ IL17+')

#identify all the possible combinations of interacting cells 
all.combinations <- unique(expand.grid(list3_vedo2, list3_vedo2))

#define the colnames in the created combinations dataframe
colnames(all.combinations) <- c('celltype1', 'celltype2')

#create a union between the teo names of all possible combinations to define the output folder 
all.combinations$folder.out <- paste0(all.combinations$celltype1, '_', all.combinations$celltype2)

#make the names of the folder safe
all.combinations$folder.out <- gsub('\\+', 'pos', all.combinations$folder.out)
all.combinations$folder.out <- gsub('- ', 'neg_', all.combinations$folder.out)
all.combinations$folder.out <- gsub(' |-', '_', all.combinations$folder.out)

#define the main folder output
folder.main <- '/groups/umcg-weersma/tmp01/projects/vedopredict2/biopsies/ongoing/NicheNetPairwise/new_results/'

#run the analysis
for (i in 1:nrow(all.combinations)) {
  tryCatch(
    {
      receiver = all.combinations[i, 'celltype1']
      print(receiver)
      ...
      ## sender
      sender_celltypes = all.combinations[i, 'celltype2']
      Idents(object = object.sub) <- 'celltype_final.v1'
      expressed_genes_sender = get_expressed_genes(sender_celltypes, object.sub, pct = 0.10)
      
      print(paste0('doing ',receiver, ' and ', sender_celltypes))
      ...
      #create the directory for the couple of celltypes
      dir.create(file.path(folder.main, all.combinations[i, 'folder.out']))
      
      ggsave(paste0(folder.main, all.combinations[i, 'folder.out'], '/dotplot.pdf'), width= 25, height = 15)
      
      print(paste0('saved dotplot for ', receiver, ' and ', sender_celltypes))
      ...
      ggsave(paste0(folder.main, all.combinations[i, 'folder.out'],  '/p_ligand_target_network.pdf'), width= 10, height = 10)
      
      print(paste0('saved p_ligand_target_network for ', receiver, ' and ', sender_celltypes))
      ...
      ggsave(paste0(folder.main, all.combinations[i, 'folder.out'],  '/p_ligand_receptor_network.pdf'), width= 25, height = 15)
      
      print(paste0('saved p_ligand_receptor_network for ', receiver, ' and ', sender_celltypes))
      ...
      ggsave(paste0(folder.main, all.combinations[i, 'folder.out'],  '/p_ligand_receptor_network_strict.pdf'), width= 5, height = 5)
      
      print(paste0('saved p_ligand_receptor_network_strict for ', receiver, ' and ', sender_celltypes))
      ...
      ggsave(paste0(folder.main, all.combinations[i, 'folder.out'],  '/p_ligand_lfc.pdf'), width= 10, height = 10)
      
      print(paste0('saved p_ligand_lfc for ', receiver, ' and ', sender_celltypes))
      
    },
    error=function(cond) {
      message(paste0("There is an issue with this celltype:",  receiver, ' and ', sender_celltypes))
