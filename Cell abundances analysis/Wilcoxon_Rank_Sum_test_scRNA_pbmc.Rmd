---
title: "Wilcoxon rank sum test_scRNA pbmc"
...
# using Wilcoxon Rank Sum test in scRNA-seq pbmc data
# Date: 09-Sep-2022
# Author: Shiqiang Sun
##########################################################################################
```
  ...
  tryCatch(
    {
      ...
      p_value <- sapply(counts[1:31], function(x) wilcox.test(x ~ counts[[response]], exact = FALSE, paired = F)$p.value)
  ...
  tryCatch(
    {
      ...
      # Run Wilcoxon Rank Sum test
      p_value <- sapply(counts[1:31], function(x) wilcox.test(x ~ counts[[timepoint]], exact = FALSE, paired = F)$p.value)
      # Change p_value as dataframe
      p_value <- as.data.frame(p_value)
      # select significant results
      p_value_0.05 <- filter(p_value, p_value < 0.05)
...
output_dir <- "/groups/umcg-weersma/tmp01/projects/vedopredict2/pbmc/ongoing/Wilcoxon_rank_sum_test/"
...
Wilcoxon_Rank_Sum_test_1(vedo2_scRNA_pbmc_counts, "T0", "PGA_response")

# 2. after_nonresponders vs after_responders (after_responders as reference)
Wilcoxon_Rank_Sum_test_1(vedo2_scRNA_pbmc_counts, "T4", "PGA_response")

# 3. after_responders vs before_responders (before_responders as reference)
Wilcoxon_Rank_Sum_test_2(vedo2_scRNA_pbmc_counts, "yes", "timepoint")

# 4. after_nonresponders vs before_nonresponders (before_nonresponders as reference)
Wilcoxon_Rank_Sum_test_2(vedo2_scRNA_pbmc_counts, "no", "timepoint")
